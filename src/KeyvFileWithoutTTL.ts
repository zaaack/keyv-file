...
export class KeyvFileWithoutTTL extends EventEmitter implements KeyvStoreAdapter {
  ...
  constructor(options?: Partial<Options>) {
    ...
    try {
      let data = this.opts.deserialize(fs.readFileSync(this.opts.filename, 'utf8'))
      if (!Array.isArray(data)) {
        const _cache = data
        data = []
        for (const key in _cache) {
          if (_cache.hasOwnProperty(key)) {
            data.push([key, _cache[key]])
          }
        }
      }
  ...
  private saveToDisk() {
    ...
    const data = this.opts.serialize(cache)
...
export default KeyvFileWithoutTTL
